...
public interface IUserService {
    ...
    List<Song> findAllSong (Integer uid, Integer listId);
     ...
     */
    boolean addUser (User user, String password) throws NullUserException, UidErrorException, UserPasswordNotValidException, UserPhoneNotValidException, UserInfoNotValidException;
    void addAdministrator ();
    /**
     * 修改用户的用户名、性别、地区和生日信息或者修改用户 vip 属性，依据 action 的值调用对应的方法
     * 其中 vip 属性是将其修改为当前的非值：true -> false, false -> true
     * @param user 需要修改信息的用户对象
     * @param action 枚举值，应为 updateUserInfo 或者 updateUserVip
     ...
     */
    boolean updateUser (User user, UpdateUserAction action) throws NullUserException, UidErrorException, UserPhoneNotValidException, UserInfoNotValidException;
    ...
    boolean updateSongList (Integer uid, Integer listId, Integer songId, SongAction action);
    
    boolean updateSongListName (Integer uid, Integer listId, String listName);
    
    boolean updateFollow (Integer uid, Integer fUid, FollowAction action);
